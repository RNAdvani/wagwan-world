...
func (h *GuestHandler) CreateGuest(w http.ResponseWriter, r *http.Request) {
	...
	var notes string = ""
	var eventID int  = 1 // To ensure backward compatibility it is set to Legacy Event ID
	var plusOnes int = 0
	...
	guest, err := db.CreateGuest(h.db, req.Name, req.Email, req.Phone, req.Status, notes, eventID, plusOnes, dietaryRestrictions)
	if err != nil {
